...
var addclasses = function addclasses(elem, classes) {
...
*/
function _onChange() {
    var paths = window.location.hash.substring(1),
        pathname = paths.split('/');
    var slide = document.getElementById('slid');
    if (paths !== '') {
        _getAjx("/partial/" + paths).then(function (response) {
...
function togglepay(evt) {
    ...
    var slide = document.getElementById('slid'),
        billername = theElem.getAttribute('data-name'),
        category = theElem.getAttribute('cat-name'),
...
function _onalert() {
    if (location.search !== '') {
        ...
        if (querstring[1].split('=')[0] === 'e') {
            //the querstring second element get its key i.e. su->success or e ->error
